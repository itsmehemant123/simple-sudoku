...
15. **Hints use derived candidates, not player notes.** The in-game hint UI (`Game.updateHints` / `Game.nextMove`) runs `Techniques.deriveCandidates(game.puzzle, game.entries)` (values read as `entries[i] || puzzle[i]`) and `Techniques.findHintsForCell` / `Techniques.findHint`. Detection never reads or mutates the player's `candidates` arrays, and the overlay never changes the board or notes — it is informational only (an on/off toggle in the game view; taps keep working as Value/Candidate). Highlight marks (`.hint-cell` pattern, `.hint-elim` elimination, `.hint-key` focal cell, `.elim-digit` struck digit in an elim cell's candidate grid) are applied **only to cells that hold a value or the player's own candidates** — completely blank cells get no marks (they're still named in the panel text in plain "row N, column M" words, locatable via the Indexes toggle). Because a hint on a blank cell is otherwise invisible, `Game.nextMove` additionally **selects the hint's focal cell** (its `keyCells[0]`, else `cells[0]`) so the row/column/box influence highlights make it easy to find — do not remove that selection, or Next Move becomes confusing on a fresh board. The **Next Move button is part of the hints feature and is hidden unless the Hints toggle is on** (`nextMoveBtn.classList.toggle('hidden', !hintsOn)` in `toggleHints` and `enterGame`). Never add the bare `.hint` class to a board cell (it is the muted paragraph helper and sets `margin: 8px 0 0`); the pattern highlight class is `.hint-cell`. The `#hint-panel` (replaced the old `#hint-message`) always reserves vertical space via `.hint-panel` `min-height` (so the pinned compact board never shifts when a hint appears/clears), and `.cell` hover styling is scoped to `@media (hover: hover)` so touch taps don't trigger iOS compositing glitches. Friendly explanations come from `Techniques.hintInfo(hint)` (`{ name, steps, blurb }`) — the glossary `TECHNIQUE_INFO` and `steps` wording live in `js/techniques.js`, not in the UI.
